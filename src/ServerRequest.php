...
use NeoP\Http\Message\Uploaded\UploadedFile;
use NeoP\Http\Message\Stream\Stream;
use NeoP\Http\Message\Uri\Uri;
use NeoP\Http\Message\Message;
...
class ServerRequest extends Message implements ServerRequestInterface
{
    ...
    public function __construct(SwooleRequest $swooleRequest)
    {
        ...
        $this->withBody(new Stream((string) $swooleRequest->rawContent()));
        $this->withUri(new Uri($uri));
        $this->withRequestTarget($uri);
        $this->withProtocolVersion($protocolVersion);
        $this->withHeaders($swooleRequest->header);
    ...
    public function getCurrentParsedBody()
    {
        $data = $this->getPostParams();

        $contentType = strtolower($this->getHeaderLine('Content-Type'));
        if (strpos($contentType, 'application/json') === 0) {
            $data = json_decode($this->getBody()->getContents(), true) ?? $data;
        }
